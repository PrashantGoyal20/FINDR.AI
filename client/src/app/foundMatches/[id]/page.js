...
const page = () => {
  ...
  useEffect(()=>{
    const fetchFoundItems = async () => {
      await axios.get(`/api/matchFound/${id}`,{withCredentials: true}).then((res)=>{
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-6 sm:mb-8 space-y-4 sm:space-y-0">
            <div>
              <h1 className={`text-3xl sm:text-4xl lg:text-5xl font-bold ${themeClasses.text.primary} mb-2`}>
                Matches for your Reported Item
              </h1>
              ...
              {foundItems.map((item) => (
                ...
                >
                  ...
                  <div className="flex items-center justify-between pt-4 border-t border-gray-700/30">
                    ...
                    <button onClick={()=>router.push(`/lostMatchDetail/${item.id}`)} className={`px-4 py-2 flex items-center rounded-lg font-medium text-sm transition-all duration-300 ${themeClasses.button.primary}`}>
