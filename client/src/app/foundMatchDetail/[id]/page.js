...
const Page = () => {
  ...
  useEffect(()=>{
    const fetchData=async()=>{
      setLoading(true)
      await axios.get(`/api/foundMatchDetail/${id}`,{withCredentials:true}).then((res)=>{
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className={`p-6 sm:p-8 rounded-2xl sm:rounded-3xl border shadow-2xl ${themeClasses.card}`}>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
                ...
                <button className={`flex items-center justify-center px-4 py-3 rounded-xl transition-all duration-300 border ${themeClasses.button.secondary} ${themeClasses.text.primary}`}>
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex items-start">
                    ...
                    <div>
                      ...
                      <div className={themeClasses.text.secondary}>{formatDate(item.date_lostcreated_at)}</div>
                      <div className={`text-sm ${themeClasses.text.muted}`}>{getTimeSince(item.date_lostcreated_at)}</div>
                    </div>
                  </div>
              ...
              {item.reward && (
                <div className="mt-6 p-4 rounded-xl bg-blue-500/10 border border-blue-500/20">
                  <div className="flex items-start">
                    ...
                    <div>
                      <div className={`font-semibold mb-1 ${themeClasses.text.primary}`}>Additional Information</div>
                      <p className={`text-sm ${themeClasses.text.secondary}`}>{item.reward}</p>
                    </div>
                  </div>
                </div>
              )}
        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-fade-in">
          <div className={`max-w-md w-full p-6 sm:p-8 rounded-3xl border shadow-2xl ${themeClasses.card} animate-scale-in`}>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <button className={`py-3 rounded-xl font-semibold transition-all duration-300 ${themeClasses.button.primary}`}>
                Send Message
              </button>
            </div>
          </div>
