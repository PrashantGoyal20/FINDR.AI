...
const page = () => {
  ...
  useEffect(()=>{
    const fetchFoundItems = async () => {
      await axios.get('/api/allFoundItems',{withCredentials: true}).then((res)=>{
  ...
  const stats = {
    total: foundItems.length==0?'0':foundItems.length,
    active: foundItems.length==0?'0':foundItems.filter(i => i.status === 'active').length,
    found: foundItems.length==0?'0':foundItems.filter(i => i.status === 'found').length,
    totalMatches: foundItems.reduce((sum, item) => sum + item.matches, 0)
  };
        ...
        <div className="relative z-10 container mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-8">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-6 sm:mb-8 space-y-4 sm:space-y-0">
            <div>
              ...
              <p className={`text-sm sm:text-base ${themeClasses.text.secondary}`}>
                Help owners find their lost belongings 
              </p>
            </div>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-6 mb-6 sm:mb-8">
            ...
            <div className={`p-4 sm:p-6 rounded-2xl sm:rounded-3xl border shadow-xl ${themeClasses.card}`}>
              ...
              <div className={`text-xs sm:text-sm ${themeClasses.text.secondary}`}>Claimed</div>
            </div>
          ...
          {foundItems.length === 0 ? (
            <div className={`p-12 sm:p-20 rounded-3xl border shadow-xl text-center ${themeClasses.card}`}>
              ...
              {!searchQuery && filterStatus === 'all' && (
                <button onClick={()=>router.push('/foundLost')} className={`px-6 py-3 rounded-xl font-semibold transition-all duration-300 hover:scale-105 shadow-lg ${themeClasses.button.primary}`}>
